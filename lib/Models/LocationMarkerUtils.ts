...
): CzmlCatalogItem
{
    ...
    const marker = {
        ...
        description: `<table><tr><td>${i18next.t("featureInfo.latLon")}</td><td>${displayCoords.latitude
    ...
    {
        console.log(catalogItem.czmlData);
        catalogItem.setTrait(CommonStrata.user, "czmlData", [...catalogItem.czmlData, marker]);
    }
...
export function removeMarker(terria: Terria)
{
    ...
    if (catalogItem !== undefined)
    {
        ...
        catalogItem.setTrait(CommonStrata.user, "czmlData", [catalogItem.czmlData[0]]);
    }
}
